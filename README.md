...
* bl2_bp-msrzg2ul.bin - ARM Boot Loader stage 2 (MSRZG2UL only)
* bl2_bp-msrzg2ul.srec - ARM Boot Loader stage 2, Motorola S-record format (MSRZG2UL only)
* fip-msrzg2ul.bin - combined U-Boot image (MSRZG2UL only)
* fip-msrzg2ul.srec - combined U-Boot image, Motorola S-record format (MSRZG2UL only)
...
U-Boot for MSRZG2UL/MSRZFive includes builin support for network interfaces. Thus when initial programming of U-Boot is done, additional images such as Linux Kernel and rootfs can be loaded via network. MSRZG2UL/MSRZFive targets has 16 Mb of QSPI Flash. This is enough for storing a Linux Kernel and initramfs images. While a full fledged root filesystem can be created on a SD card, or USB flash drive.
...
1. Copy U-Boot and secondary loader images (MSRZFive):
   ...
   ```
1. Copy U-Boot and secondary loader images (MSRZG2UL):
   ```
   # cp bl2_bp-msrzg2ul.bin /var/lib/tftpboot/boot/
   # cp fip-msrzg2ul.bin /var/lib/tftpboot/boot/
   ```
...
1. Mount the root file system on the host, and copy kernel and DTB images into the `/boot` directory on the file system:
   ...
   ```
1. Insert the SD/USB drive into the MSRZG2UL/MSRZFive target and reset it.
